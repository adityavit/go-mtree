...
func modeUpdateKeywordFunc(path string, value string) (os.FileInfo, error) {
	vmode, err := strconv.ParseInt(value, 8, 32)
	if err != nil {
		return nil, err
	}
	Debugf("value: %q, vmode: %o", value, vmode)
...
func tartimeUpdateKeywordFunc(path string, value string) (os.FileInfo, error) {
...
func timeUpdateKeywordFunc(path string, value string) (os.FileInfo, error) {
	v := strings.SplitN(value, ".", 2)
	if len(v) != 2 {
